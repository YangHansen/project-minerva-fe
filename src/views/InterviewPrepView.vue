...
import { ArrowLeft, BarChart3, CheckCircle2, CircleStop, Clock3, Mic, Pause, Play, Sparkles, Video } from 'lucide-vue-next'
...
const submitRecordedAnswer = async (audio: Blob, durationSeconds: number) => {
  ...
  try {
    ...
    answerResults.value = {
      ...answerResults.value,
      [activeQuestion.id]: { transcript, evaluation: result.evaluation, durationSeconds },
    }
...
<button class="btn-primary" :disabled="generatingInterview" @click="start">{{ generatingInterview ? 'Preparing questions…' : 'Start interview' }} <Play :size="16" />
</button>
</div>
...
<small class="absolute bottom-4 left-4 font-bold text-slate-500">Your local camera preview · video is not uploaded</small>
</div>
