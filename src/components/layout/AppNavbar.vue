...
<template>
  <header class="sticky top-0 z-50 border-b border-slate-200/70 bg-white/90 backdrop-blur-xl">
    <nav class="mx-auto flex h-18 w-[min(1180px,calc(100%-40px))] items-center justify-between" aria-label="Primary navigation">
      <RouterLink to="/" class="flex items-center font-extrabold text-[#17136b]" aria-label="Minerva home">
        <img src="/minerva-logo.png" alt="Minerva" class="h-14 w-48 object-cover">
      </RouterLink>
      ...
      <div class="hidden items-center gap-3 lg:flex">
        <RouterLink to="/login" class="inline-flex min-h-11 items-center justify-center rounded-xl px-4 text-sm font-extrabold text-[#17136b] hover:bg-violet-50">Log In</RouterLink>
        <RouterLink to="/register" class="inline-flex min-h-11 items-center justify-center rounded-xl bg-[#5b45f5] px-4 text-sm font-extrabold text-white shadow-[0_10px_24px_rgba(91,69,245,.2)] transition-colors hover:bg-[#4c35d9]">Get Started</RouterLink>
      </div>
    ...
    <Transition enter-active-class="transition duration-200" enter-from-class="-translate-y-2 opacity-0" leave-active-class="transition duration-200" leave-to-class="-translate-y-2 opacity-0">
      <div v-if="open" id="mobile-menu" class="border-t border-slate-200 bg-white p-5 lg:hidden">
        <div class="mx-auto grid w-[min(1180px,calc(100%-40px))] gap-1">
          ...
          <div class="mt-3 grid grid-cols-2 gap-3"><RouterLink to="/login" class="inline-flex min-h-11 items-center justify-center rounded-xl border border-slate-200 bg-white px-4 text-sm font-extrabold text-[#17136b]">Log In</RouterLink><RouterLink to="/register" class="inline-flex min-h-11 items-center justify-center rounded-xl bg-[#5b45f5] px-4 text-sm font-extrabold text-white shadow-lg">Get Started</RouterLink></div>
        </div>
      </div>
    </Transition>
  </header>
